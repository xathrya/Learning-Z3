# Solve a crackme using Z3
#
# Use: 
#    python day15.py 
# 

# Taken from https://github.com/sam-b/z3-stuff/blob/master/hackvent-15/15.py
...
# Each digit is a decimal number
solver.add(b < 10, b >= 0)
solver.add(c < 10, c >= 0)
solver.add(d < 10, d >= 0)
solver.add(f < 10, f >= 0)
...
# Each number is consisting some digits
...
# Creating constraint for some numbers
solver.add(bytwycju + yzvyjjdy ^ vugljtyn + ugdztnwv | xbfziozy == bzuwtwol)
solver.add(wwnnnqbw - uclfqvdu & oncycbxh | oqcnwbsd ^ cgyoyfjg == vyhyjivb)
solver.add(yzdgotby | oigsjgoj | ttligxut - dhcqxtfw & szblgodf == sfgsoxdd)
solver.add(yjjowdqh & niiqztgs + ctvtwysu & diffhlnl - thhwohwn == xsvuojtx)
solver.add(nttuhlnq ^ oqbctlzh - nshtztns ^ htwizvwi + udluvhcz == syhjizjq)
solver.add(bytwycju ^ wwnnnqbw & yzdgotby + yjjowdqh - nttuhlnq == fjivucti)
solver.add(yzvyjjdy ^ uclfqvdu & oigsjgoj + niiqztgs - oqbctlzh == zoljwdfl)
solver.add(vugljtyn ^ oncycbxh & ttligxut + ctvtwysu - nshtztns == sugvqgww)
solver.add(ugdztnwv ^ oqcnwbsd & dhcqxtfw + diffhlnl - htwizvwi == uxztiywn)
solver.add(xbfziozy ^ cgyoyfjg & szblgodf + thhwohwn - udluvhcz == jqxizzxq)
